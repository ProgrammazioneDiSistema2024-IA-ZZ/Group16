...
pub fn play_sound(number: i32) {
    ...
    if number == 0 {
        file = File::open(env::current_exe().unwrap().parent().unwrap().parent().unwrap().join("assets/blip-131856.mp3")).unwrap();
    }else if number == 1 {
        file = File::open(env::current_exe().unwrap().parent().unwrap().parent().unwrap().join("assets/success-48018.mp3")).unwrap();
    } else {
        file = File::open(env::current_exe().unwrap().parent().unwrap().parent().unwrap().join("assets/stop-13692.mp3")).unwrap();
