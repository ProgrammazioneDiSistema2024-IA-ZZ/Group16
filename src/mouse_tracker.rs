...
fn is_near(p1: &Point, p2: &Point, tolerance: f64) -> bool {
...
fn contains_corners(
    ...
    if !enable {
        ...
        for point in points {
            if is_near(&point, &top_left, tolerance) {
                found_top_left = true;
            }
            if is_near(&point, &top_right, tolerance) {
                found_top_right = true;
            }
            if is_near(&point, &bottom_left, tolerance) {
                found_bottom_left = true;
            }
            if is_near(&point, &bottom_right, tolerance) {
                found_bottom_right = true;
            }
        }

        found_top_left && found_top_right && found_bottom_left && found_bottom_right
    } else {
